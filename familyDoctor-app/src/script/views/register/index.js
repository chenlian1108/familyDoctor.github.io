...
export default class LoginNP extends Component {
    ...
    userRegister = () => {
        ...
        nickName && password && axios.post("/react/register", {
            nickName,
            password
        }).then(res => {
            console.log(res);
            if (!!res.data.type) {
                this.props.history.push("/main/nplogin");
                var userInfo = {
    ...
    render() {
        ...
        return (
            <div>
                <Head title="用户注册" show={true} showsearch={false}></Head>
                ...
                <List>
                    ...
                    <WingBlank>
                        <WhiteSpace size="xl" />
                        <Button type="primary" onClick={this.userRegister} disabled={toggle}>注册</Button>
                    </WingBlank>
